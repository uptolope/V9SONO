---
title: "SonoPrep Blog — Free SPI Exam Study Guides"
description: "Free in-depth study guides for ARDMS SPI exam candidates. Ultrasound physics, Doppler principles, artifact recognition, study strategy, and credential comparisons."
url: "https://sonoprep.com/blog"
---

# SonoPrep Blog — Free SPI Exam Study Guides

Free, in-depth study resources for ARDMS SPI exam candidates. Every article is written specifically for the SPI — not generic ultrasound education.

---

## [The Complete ARDMS SPI Exam Guide: Everything You Need to Pass](https://sonoprep.com/blog/complete-spi-exam-guide)
*May 12, 2026 · 25 min read*

The definitive guide to the SPI exam. Covers what the exam tests, the five domains and their weights, a proven 6-week study plan, the most common mistakes, and how to approach each domain strategically.

---

## [ARDMS SPI Exam Blueprint: Domain Weightings Explained](https://sonoprep.com/blog/ardms-exam-blueprint)
*April 26, 2026 · 17 min read*

A deep dive into the official ARDMS SPI content outline — exactly how many questions come from each domain, what subtopics within each domain are tested, and how to weight your study time accordingly.

---

## [Doppler Principles for the SPI Exam: Nyquist, Aliasing & Spectral Analysis](https://sonoprep.com/blog/doppler-principles-spi-exam)
*April 18, 2026 · 20 min read*

Doppler is 34% of the SPI exam. This guide covers every core concept ARDMS tests: the Doppler effect and its applications, continuous wave vs. pulsed wave Doppler, the Nyquist limit, aliasing and how to correct it, PRF and wall filter, and spectral Doppler waveform analysis.

---

## [Ultrasound Physics for the SPI Exam: The 6 Concepts That Actually Appear](https://sonoprep.com/blog/ultrasound-physics-spi)
*April 10, 2026 · 18 min read*

Cut through the noise. These are the foundational ultrasound physics concepts responsible for the most exam questions: frequency/wavelength/velocity relationships, the four resolution types, attenuation, the piezoelectric effect, PRF and duty factor, and acoustic output.

---

## [Ultrasound Artifacts: The 7 Most Common SPI Exam Questions](https://sonoprep.com/blog/ultrasound-artifacts-spi)
*April 3, 2026 · 15 min read*

Artifact recognition is heavily tested on the SPI. This guide covers the 7 artifacts most frequently appearing on the exam: reverberation, shadowing, enhancement, comet-tail, mirror image, refraction, and side lobe.

---

## [How to Pass the SPI Exam on Your First Attempt: A 6-Week Blueprint](https://sonoprep.com/blog/pass-spi-first-attempt)
*March 28, 2026 · 14 min read*

A week-by-week study schedule built around the ARDMS SPI content outline and weighted by domain. Designed to get first-time candidates to passing within six weeks of focused study.

---

## [Spaced Repetition for SPI Exam Success: Why Active Recall Beats Cramming](https://sonoprep.com/blog/spaced-repetition-spi-exam)
*March 20, 2026 · 12 min read*

The neuroscience of spaced repetition and why it works better than re-reading for physics content. Covers the spacing effect, active recall, how to apply both to ARDMS SPI domains, and how SonoPrep's flashcard system is built around these principles.

---

## [SPI Exam Test-Taking Strategies: Eliminate Wrong Answers & Manage Time](https://sonoprep.com/blog/test-taking-strategies-spi)
*March 14, 2026 · 11 min read*

Practical strategies for the exam itself: how to parse ARDMS-style question stems, how to systematically eliminate distractor answers, pacing through 110 questions in 2.5 hours, and how to handle questions where two answers look equally correct.

---

## [RDMS vs RDCS vs RVT vs RMSKS: Which ARDMS Specialty Is Right for You?](https://sonoprep.com/blog/ardms-specialties-comparison)
*March 6, 2026 · 13 min read*

A practical comparison of the four main ARDMS specialty credentials: what each one covers, what the corresponding specialty exam tests, typical career paths, and how to decide which credential to pursue after passing the SPI.
