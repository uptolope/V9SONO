...
const PRODUCTS = [
  { name: "SPI Flashcards", price: "$29", description: "Master key physics concepts with SM-2 spaced repetition.", features: ["200+ expert-written flashcards", "SM-2 spaced repetition algorithm", "Progress tracking", "Mobile optimized"] },
  ...
  { name: "Premium Bundle", price: "$99", description: "Everything you need — all four products at a discounted price.", features: ["All 4 products included", "200+ flashcards", "170-question simulator", "159-page notes", "50 Physics Pearls", "Save $27"], featured: true, bundle: true }
];

export default function ProductsPage() {
  return (
    <div className="min-h-screen pt-36 px-6"> {/* increased top padding */}
          ...
          {PRODUCTS.map((product) => (
            <div key={product.name} className={`depth-border corner-arch p-6 tactile-card flex flex-col ${product.featured ? "border-l-[3px] border-l-[#c85b3a]" : ""} ${product.bundle ? "bg-[#c85b3a]/5" : ""}`}>
              ...
              <div className="pt-8 flex-grow">
                ...
                <div className="text-3xl font-bold text-[#c85b3a] mb-2">{product.price}<span className="text-xs text-[#6b6359] ml-2">/ 90-day access</span></div>
              ...
              <Link href="/api/checkout" className={`inline-block w-full text-center py-3 transition-all duration-300 tactile-button ${product.bundle ? "bg-[#c85b3a] text-white hover:bg-[#a8452a]" : "border border-[#c85b3a]/50 text-[#f0ebe4] hover:bg-[#c85b3a]/10"}`}>GET {product.bundle ? "BUNDLE →" : "STARTED →"}</Link>
            </div>
          ))}
        </div>
